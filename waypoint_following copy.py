...
# ── 하이퍼파라미터 / 상수 ────────────────────────────────────
Kp, Ki, Kd       = 1.1, 0.05, 0.2               # PID gains
    ...
    try:
        while True:
            ...
            if (Task1_Flag):  
                # Task1: 신호등 인식
                print(f"Task1 Start!!")
                if (depths[RED] is not None and depths[RED] > 4):

                    TASK_SPEED = 0
                    print(f"[SIGNAL] RED LIGHT DETECTED! STOP!")
                    Task1_Red_Flag = 1
                   
                elif (depths[GREEN] is not None and Task1_Red_Flag == 1):
            ...
            # comment : 그냥 depth trigger 걸렸을 때 시간 재고 해당 몇초 지나면 slow 랑 stop 알아서 풀리게끔
                ...
                print(f"Task2 Start!!")
                if (depths[SLOW] is not None and depths[SLOW] > 0):
                    TASK_SPEED = 50
                ...
                elif (depths[STOP] is not None and depths[STOP] > 0):
                    TASK_SPEED = 0
                ...
                # comment : 사다리꼴
                print(f"Task3 Start!!")
                if (depths[VEHICLE] is not None and depths[VEHICLE] > 4):
                ...
                if (depths[RED] is not None and depths[RED] > 4): # 빨간불인 경우 정지
                    TASK_SPEED = 0
                    print(f"[SIGNAL] RED LIGHT DETECTED! STOP!")
                    Task4_Red_Flag = 1

                elif depths[GREEN] is not None and depths[GREEN] > 4:                                             # 빨간불이 아니고 초록불인 경우  
                    if (depths[LEFT] is not None and depths[LEFT] > 5):
                        send_control(-10,200)
                        time.sleep(3)
                    elif (depths[RIGHT] is not None and depths[RIGHT] > 5):
                        send_control(200,-10)
                        time.sleep(3)
                    elif (depths[STRAIGHT] is not None and depths[STRAIGHT] > 5):
                        TASK_SPEED = BASE_SPEED
                    else:
                        TASK_SPEED = BASE_SPEED
            ...
            # if abs(steer) > 0.15 and Task1_Red_Flag == 0:
