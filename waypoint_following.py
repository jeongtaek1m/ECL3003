...
# ── 하이퍼파라미터 / 상수 ────────────────────────────────────
Kp, Ki, Kd       = 1.0, 0.0, 0.2               # PID gains
UPDATE_INTERVAL  = 0.10                        # [s]
BASE_SPEED       = 100                         # 기본 속도 [%]
MAX_STEER_RATIO  = 0.8                         # |steer|=1 일 때 감속비
...
model.load_state_dict(torch.load('./Lane_model/last_lane.pt',
                                 weights_only=True))
...
def compute_motor_output(steer: float, base_speed: float):
    ...
    if steer < 0:          # 우회전 → 왼쪽 감속
        L = base_speed * (1.0 - turn_scale)
        R = base_speed * 2.0
    elif steer > 0:        # 좌회전 → 오른쪽 감속
        L = base_speed * 2.0
...
def infer_waypoint():
    ...
    with torch.no_grad():
        output = model(preprocess(image_pil)).cpu().numpy()[0]  # (x,y)
...
def run_pid_loop():
    try:
        while True:
            ...
            print(f"[WAYPOINT] x={x:.1f}, error={lateral_error:+.2f}, "
